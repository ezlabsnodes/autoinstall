...
if ! fallocate -l "$SWAP_SIZE" "$SWAPFILE"; then
    ...
    if [[ "$SWAP_SIZE" =~ ^([0-9]+)[Gg]$ ]]; then
        ...
        dd if=/dev/zero of="$SWAPFILE" bs=1G count="$COUNT" status=progress || error "Failed to create swapfile with dd (GiB)."
    elif [[ "$SWAP_SIZE" =~ ^([0-9]+)[Mm]$ ]]; then
        COUNT="${BASH_REMATCH[1]}"
        dd if=/dev/zero of="$SWAPFILE" bs=1M count="$COUNT" status=progress || error "Failed to create swapfile with dd (MiB)."
    else
